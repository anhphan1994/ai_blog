...
class AIController extends Controller
{
    ...
    //     $sections = explode("\n", $first_response);
    //     $sections_arr = array_slice($sections, 1);
    ...
    public function testCreatePost2()
    {
        ...
        $first_prompt = $this->generateFirstPrompt($short_desc, $keyword, $style, $num_of_section);
        ...
        $second_prompt = $this->generateSecondPrompt($title, $short_desc, $keyword, $style, $num_of_section);
        ...
        if (Cache::has('content')) {
            ...
            foreach ($sections_arr  as $index => $section) {
                if($index == count($sections_arr) - 1){
                    break;
                }
                $section_title =  $section_title ."\n".$section;
                $main_prompt = $this->generateMainPrompt($title, $short_desc, $keyword, $list_sections, $section_title);
    ...
    private function generateFirstPrompt($short_desc, $keyword, $style, $num_of_section){
        return "
            #あなたの役割
            読者100万人を超える超人気SEOブロガー。
    ...
    private function generateSecondPrompt($title, $short_desc, $keyword, $style, $num_of_section){
        return "
            #あなたの役割
            SEOの原則に基づいて効果的なブログタイトル、本文を作成するプロフェッショナルです。私の目的は、検索エンジンでの上位表示を狙い、サイトのトラフィックとエンゲージメントを増加させるための魅力的なブログ記事を作成することです。
    ...
    private function generateMainPrompt($title, $short_desc, $keyword, $list_sections, $section_title){
        return "
                SEOの原則に基づいて効果的なブログタイトル、本文を作成するプロフェッショナルです。私の目的は、検索エンジンでの上位表示を狙い、サイトのトラフィックとエンゲージメントを増加させるための魅力的なブログ記事を作成することです。
